...
resource "aws_cloudwatch_metric_alarm" "euw1_cloudwatch_kinesis_get_records_success" {
  ...
  alarm_name          = "${var.eu-west-1__kinesis__GetRecordsSuccess[count.index].ResourceName}_GetRecordsSuccess_alarm"
  ...
  statistic           = "Maximum"
  dimensions = {
    StreamName = var.eu-west-1__kinesis__GetRecordsSuccess[count.index].ResourceName
  }
...
resource "aws_cloudwatch_metric_alarm" "euw2_cloudwatch_kinesis_get_records_success" {
  ...
  alarm_name          = "${var.eu-west-2__kinesis__GetRecordsSuccess[count.index].ResourceName}_GetRecordsSuccess_alarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  ...
  statistic           = "Maximum"
  dimensions = {
    StreamName = var.eu-west-2__kinesis__GetRecordsSuccess[count.index].ResourceName
  }
