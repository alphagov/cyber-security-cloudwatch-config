# Terraform module to setup a Cloudwatch alarm to monitor the NumberOfMessagesSent for an SQS queue is 10% lower than the baseline
...
resource "aws_cloudwatch_metric_alarm" "cloudwatch_metric_alarm" {
  ...
  metric_query {
        id = "e2"
        expression = "m1*1.1"
        label = "10_percent_below_threshold"
        return_data = "true"
  }
