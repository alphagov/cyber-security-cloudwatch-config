...
resource "aws_cloudwatch_metric_alarm" "euw1_cloudwatch_firehose_execute_processing_duration" {
  ...
  alarm_name          = "${var.eu-west-1__firehose__ExecuteProcessingDuration[count.index].ResourceName}_alarm"
  ...
  metric_name         = "ExecuteProcessingDuration"
  namespace           = "AWS/Firehose"
  period              = 300
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-1__firehose__ExecuteProcessingDuration[count.index].ResourceName
  }
...
resource "aws_cloudwatch_metric_alarm" "euw2_cloudwatch_firehose_execute_processing_duration" {
  ...
  alarm_name          = "${var.eu-west-2__firehose__ExecuteProcessingDuration[count.index].ResourceName}_alarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  ...
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-2__firehose__ExecuteProcessingDuration[count.index].ResourceName
  }
