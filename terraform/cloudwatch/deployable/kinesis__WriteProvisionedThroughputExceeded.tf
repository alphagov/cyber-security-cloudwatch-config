...
resource "aws_cloudwatch_metric_alarm" "euw1_cloudwatch_kinesis_write_provision_throughput_exceeded" {
  ...
  alarm_name          = "${var.eu-west-1__kinesis__WriteProvisionedThroughputExceeded[count.index].ResourceName}_alarm"
  ...
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-1__kinesis__WriteProvisionedThroughputExceeded[count.index].ResourceName
  }
...
resource "aws_cloudwatch_metric_alarm" "euw2_cloudwatch_kinesis_write_provision_throughput_exceeded" {
  ...
  alarm_name          = "${var.eu-west-2__kinesis__WriteProvisionedThroughputExceeded[count.index].ResourceName}_alarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  ...
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-2__kinesis__WriteProvisionedThroughputExceeded[count.index].ResourceName
  }
