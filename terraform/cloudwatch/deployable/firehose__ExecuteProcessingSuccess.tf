...
resource "aws_cloudwatch_metric_alarm" "euw1_cloudwatch_firehose_execute_processing_success" {
  ...
  alarm_name          = "${var.eu-west-1__firehose__ExecuteProcessingSuccess[count.index].ResourceName}_alarm"
  ...
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-1__firehose__ExecuteProcessingSuccess[count.index].ResourceName
  }
...
resource "aws_cloudwatch_metric_alarm" "euw2_cloudwatch_firehose_execute_processing_success" {
  ...
  alarm_name          = "${var.eu-west-2__firehose__ExecuteProcessingSuccess[count.index].ResourceName}_alarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  ...
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-2__firehose__ExecuteProcessingSuccess[count.index].ResourceName
  }
