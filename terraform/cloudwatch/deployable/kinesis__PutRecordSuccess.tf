variable "eu-west-1__kinesis__PutRecordSuccess" {
  description = "List of metrics derived from aws cloudwatch list-metrics"
  type        = list
  default     = []
}

variable "eu-west-2__kinesis__PutRecordSuccess" {
  description = "List of metrics derived from aws cloudwatch list-metrics"
  type        = list
  default     = []
}

resource "aws_cloudwatch_metric_alarm" "euw1_cloudwatch_kinesis_put_record_success" {
  ...
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-1__kinesis__PutRecordSuccess[count.index].ResourceName
  }
...
resource "aws_cloudwatch_metric_alarm" "euw2_cloudwatch_kinesis_put_record_success" {
  ...
  alarm_name          = "${var.eu-west-2__kinesis__PutRecordSuccess[count.index].ResourceName}_alarm"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 2
  ...
  statistic           = "Maximum"
  dimensions = {
    QueueName = var.eu-west-2__kinesis__PutRecordSuccess[count.index].ResourceName
  }
